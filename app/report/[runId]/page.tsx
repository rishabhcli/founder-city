...
export default async function ReportPage({ params }: ReportPageProps) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-4 px-4 py-6 md:px-8">
      ...
      <h1 className="text-3xl font-semibold tracking-tight text-white">{summary.headline}</h1>
      <p className="text-slate-300">{summary.dek}</p>
      <p className="text-sm text-slate-400">City personality: {summary.cityPersonality}</p>

      <section className="grid gap-4 rounded-[28px] border border-white/10 bg-slate-950/72 p-5 sm:grid-cols-2">
        <div>
          <h2 className="mb-2 font-semibold text-white">City Score</h2>
        ...
        <div>
          <h2 className="mb-2 font-semibold text-white">Run Metadata</h2>
          <ul className="grid gap-2 text-sm text-slate-300">
      ...
      <section className="grid gap-3 rounded-[28px] border border-white/10 bg-slate-950/72 p-5 sm:grid-cols-2">
        <div>
          <h2 className="mb-2 font-semibold text-white">District Outcomes</h2>
        ...
        <div>
          <h2 className="mb-2 font-semibold text-white">Founder Outcomes</h2>
          <ul className="grid gap-1 text-sm text-slate-300">
      ...
      <section className="grid gap-3 rounded-[28px] border border-white/10 bg-slate-950/72 p-5 sm:grid-cols-2">
        <div>
          <h2 className="mb-2 font-semibold text-white">Player Startup Outcomes</h2>
          <ul className="grid gap-1 text-sm text-slate-300">
            {summary.playerStartupOutcomes.length > 0 ? summary.playerStartupOutcomes.map((line) => (
              <li key={line} className="list-disc pl-1">
                {line}
              </li>
            )) : (
              <li className="list-disc pl-1">No player startups joined this run.</li>
            )}
          </ul>
        </div>
        <div>
          <h2 className="mb-2 font-semibold text-white">Notable Interventions</h2>
          <ul className="grid gap-1 text-sm text-slate-300">
      ...
      <section className="rounded-[28px] border border-white/10 bg-slate-950/72 p-5">
        <h2 className="mb-2 font-semibold text-white">Shareable Artifact</h2>
        <p className="text-sm text-slate-300">
          Keep this page as your post-run summary card for quick debriefs, demo days, and team retros.
        </p>
        <div className="mt-3 flex flex-wrap gap-2">
          <Link href="/play" className="inline-flex rounded-full bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-cyan-200">
            Start New Run
          </Link>
          <Link
            href={`/city/${run.roomId}`}
            className="inline-flex rounded-full border border-white/12 px-4 py-2 text-sm font-semibold text-white hover:border-cyan-300/80 hover:text-cyan-200"
          >
            Reopen Room
          </Link>
        </div>
